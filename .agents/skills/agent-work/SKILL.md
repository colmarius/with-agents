...
5. **Execute or hand off**
   - Execute the bounded next action in `index.md` in the current thread by default, following the active plan when present. Load project-specific implementation or verification skills as needed.
...
6. **Finish the work**
   - Reconcile plan checkboxes when present with observed evidence, promote reusable outcomes, and follow [Closing Completed Work](#closing-completed-work).
