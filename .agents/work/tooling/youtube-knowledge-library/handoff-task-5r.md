# Task 5r Handoff Prompt

```text
Continue `.agents/work/tooling/youtube-knowledge-library/`. Read index, Task 5 plan, progress through 5q, 5q handoff, YouTube AGENTS, catalog/manifests, and established summary patterns.

Goal: seventeenth bounded backfill batch only: one `ai-concepts --limit 2` capture command, then one complete source-checked draft for every success. Coding has zero pending and must not receive an empty capture command. Task 5 stays in progress; no overview/author synthesis or Task 6.

Evidence: 63 attempts through 5q = 56 captures + 7 durable strict-language unavailable, zero ordinary transient/fatal/throttle/TooManyRequest/stopped/retry. Corpus: 107,179 transcript words and 29,104 summary words; individual transcript range remains 241–4,350 words (18×). Keep the exact maximum two attempts/no refill; stop capture on throttle/fatal/stopped and summarize prior successes.
...
Clean preflight/help exit 0/status must match: AI 63 entries/38 captured occurrences/37 unique/23 pending/2 unavailable (`9mHKjgFMsQA`, `LG3q-sqMG0g`)/0 missing/38 drafts; coding 30/25 captured/0 pending/5 unavailable (`H5cvtoSxdxI`, `D1_wPWzNkJ4`, `rCIZflYEpEk`, `qipoQGqMCp0`, `_pLlet9Jrzc`)/0 missing/25 drafts; author 86 deduped/56 captured/23 pending/7 unavailable, synthesis missing; both overviews stale. Stop on mismatch.

Run exactly once, with no refill:
`npm run youtube:library -- capture --playlist ai-concepts --limit 2`
Expected heads: `S1uKhBd72oI`, `NyzGqn4m9Bc`. Both currently occur once in AI and zero times in Coding. Actual CLI output wins; recheck membership and selected-manifest provenance before editorial work.
...
Full-success projection: AI 40 captured occurrences/39 unique/21 pending/2 unavailable; coding unchanged at 25 captured/0 pending/5 unavailable; author 58 captured/21 pending/7 unavailable; zero missing summaries. AI overview gains two stale IDs; Coding overview gains none. Actual status and shared-membership effects override projections.

Verify final status; 40 focused Node tests; `npm run lint:fix` with only known baseline plus restored unavailable hashes; generated capture-byte drift; `npm run check`; 18-page `npm run build`; both source-only boundary `rg` checks with no matches; `git diff --check`; `git show --check`; clean tree. Append dated 5r progress with exact outcomes, sizes/anchors/provenance, unavailable/transient handling, accounting, checks/commits/deviations, cumulative evidence, and 5s sizing from the actual AI-only queue. Keep Tasks 5–6 unchecked; update index to link 5r and next derive 5s only. Commit progress/index as `record YouTube backfill batch 17 progress`.
