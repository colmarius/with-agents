...
Mario Zechner built Pi so he could inspect and change how his coding agent works. In this Mastra talk, he explains its small core and project-specific extensions, arguing that developers should be able to experiment with tools and workflows rather than accept a vendor's choices.
...
- **The field is still experimenting, so harnesses should be easy to change**: Zechner argues that nobody yet knows the ideal coding-agent shape. He wants developers and agents to be able to modify the surrounding software so they can quickly try workflows instead of waiting for vendors to choose an approach [17:36–19:27](https://www.youtube.com/watch?v=Dli5slNaJu0&t=1056s).
- **Pi keeps the core small**: Separate packages connect to model providers, run the agent loop, and provide a terminal interface or programmatic access. A short system prompt accompanies four built-in tools: read, write, edit, and bash [18:38–22:06](https://www.youtube.com/watch?v=Dli5slNaJu0&t=1118s).
- **Missing features are meant to be rebuilt as files or extensions**: Pi omits built-in MCP, sub-agents, plan mode, background bash, and to-dos. Zechner instead recommends CLI tools, tmux, `PLAN.md`, `TODO.md`, or custom TypeScript extensions that reload inside the project without restarting the agent [21:17–23:09](https://www.youtube.com/watch?v=Dli5slNaJu0&t=1277s).
