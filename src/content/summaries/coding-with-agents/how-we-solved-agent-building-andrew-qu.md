...
Andrew Qu traces Vercel's internal data science agent, D0, from a large prompt to an agent working in a sandbox, then explains how those lessons informed the Eve framework. His argument is that familiar tools plus company-specific knowledge can matter more than elaborate orchestration.

**Source basis:** reviewed English text from YouTube's transcript panel, captured September 14, 2026. Caption kind is unknown; no audio check or translation was performed. Unambiguous product names are normalized. This is Qu's practitioner account; reported outcomes are not independently verified.
...
- **Make the approach easier to reuse.** Eve applies a Next.js-like file-system convention: skills, tools, and channels live in designated folders that the framework uses to assemble an agent. Channels connect it to the places people interact with it. Qu describes adapters for alternative infrastructure and a Vercel deployment path. The team rewrote D0 in Eve to make its instructions, skills, and tools easier to compose and iterate on. [11:35–14:15](https://www.youtube.com/watch?v=9dYcwOkpCE8&t=695s)
