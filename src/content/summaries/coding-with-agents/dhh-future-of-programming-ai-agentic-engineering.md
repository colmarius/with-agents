...
Lex Fridman interviews David Heinemeier Hansson (DHH), creator of Ruby on Rails and Omarchy and CTO of 37signals, about his rapid shift from writing code by hand to directing coding agents. The most useful tension is between DHH's report that agents wrote all new functionality in Omarchy Quattro and his account of a Basecamp experiment where individually plausible generated changes collectively damaged a mature architecture.
...
- **The practical shift was from prescribing code to describing problems**: DHH says earlier agents still needed him to choose the route and audit the result, while newer models can start from a fuzzy problem and propose the plan. This is his experience in the domains he works in, not evidence that review is optional across software categories ([00:07:08]-[00:12:45]).
- **Generated changes can still erode a mature system**: DHH reports that Omarchy Quattro's new functionality was entirely agent-written, but says Basecamp's early designer-led experiment produced PRs that looked defensible alone and damaged the architecture in aggregate. Human engineers then cleaned up the system ([00:15:14]-[00:17:38]).
...
- **Parallel agents change the interface and the cognitive load**: His terminal-centered setup moved from tmux to Herdr notifications and multiple machines, with roughly 16 active threads at his reported limit. He explicitly calls the pace exhausting and unsustainable, and expects more asynchronous automation to replace continuous task switching ([01:32:36]-[01:40:37], [02:43:27]-[02:48:41]).
...
- **Keep execution isolated from untrusted inputs**: In his autonomous Omarchy bot, DHH describes a coordinator that directs workers in isolated VMs so untrusted pull-request code and test output do not share the model's trusted environment. He also recounts the system noticing that adversarial test feedback could contaminate a later decision ([02:58:20]-[03:00:39]).

### Open Source and the Changing Role

- **Agent triage can protect the maintainer's decision bandwidth**: DHH says agents screen duplicate or unsuitable Omarchy contributions, validate candidate fixes in VMs, and bring only merge-ready decisions to him. His reported volume—more than 1,000 merged PRs and about 400 open ones—shows the scale of his own project, not that generated contributions are broadly cheaper to govern ([00:28:35]-[00:35:25]).
- **Learn the present workflow instead of forecasting distant roles**: His advice to anxious programmers is to focus on what current systems can do, build things, and retain community rather than trying to predict several model generations. He distinguishes people who enjoy creating useful software from roles centered only on manually producing assigned code, but offers no labor-market analysis that resolves how demand will change ([01:10:24]-[01:24:08]).
