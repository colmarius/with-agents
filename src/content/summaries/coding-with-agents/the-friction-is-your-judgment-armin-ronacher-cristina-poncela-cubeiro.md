...
- **A fallback can hide a serious failure**: Ronacher describes agent-written code that loads defaults when it cannot read a configuration file. In his example, the program keeps running but may write incorrect database records before anyone notices. He argues that agents optimize for making progress without the unease that would make an engineer question that fallback [06:30–08:25](https://www.youtube.com/watch?v=_Zcw_sVF6hU&t=390s).
