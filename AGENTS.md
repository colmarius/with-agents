...
- In the final response after a resource addition with a new or substantially revised summary, offer an optional Oracle review for readability, understandability, source fidelity, and citation coverage. Offer once per task or batch, after self-review and verification; wait for the user's agreement before invoking Oracle for this optional review. Skip the offer for metadata-only changes or when Oracle already reviewed the summary during the task. The offer is not a completion gate and does not replace required editorial review.
...
5. After drafting, complete the Summary readability review below. Resolve material comprehension and source-fidelity issues before final validation and rendered-page verification; a successful transcript capture or passing check does not complete editorial review.
...
- After splitting or expanding a passage, check every resulting claim, definition, explanation, and example against the source, including surrounding context. Check both ends of every affected citation range: it must cover the complete supporting passage, not merely the topic's introduction or one supporting chunk. Keep citations beside the claims they support; do not retain or widen an old range without reading it.

Self-review is required. Independent review is optional when conceptual density, translation, substantial revisions, or publication impact makes another read useful. Give the reviewer the summary and source; request concrete comprehension or source-fidelity findings with passage evidence, separated from wording preferences. The owning thread judges feedback and rechecks accepted edits. Consult Oracle when explicitly requested or when direct investigation leaves a specific, high-impact source-fidelity or publication judgment unresolved—not as an automatic approval gate. Keep review findings in the active conversation, not long-lived per-summary review artifacts. This does not replace source-library dedicated review or `reviewed` publication gates.
